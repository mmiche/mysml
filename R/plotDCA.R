...
#
#' @param posDodgeWidth Numeric value (default is 0), to determine how far apart point estimates and intervals of both models shall be.
#
...
#' @references
#'
#' \insertRef{ggplot22016}{mysml}
#
#' @export
#
plotDCA <- function(allDCA = NULL, bothModels=FALSE, posDodgeWidth=0) {
    
    modNames <- as.character(unique(allDCA$label))
    
    if(bothModels) {
        dca <- allDCA
        # Before it was "red", "blue", instead of mediumseagreen and magenta3
        useColor <- c("black", "darkgrey", "mediumseagreen", "magenta3")
        names(useColor) <- modNames
    } else {
        dca <- allDCA[!allDCA$label %in% modNames[length(modNames)],]
        dca$label <- droplevels(dca$label)
        useColor <- c("black", "darkgrey", "mediumseagreen")
    ...
    if(any(colnames(dca) == "lci")) {
        ...
        if(bothModels) {
            dcaPlotOverlay <-
                dcaPlot +
                geom_point(aes(y=.data$mn), size=3, position = position_dodge(width=posDodgeWidth)) +
                geom_errorbar(width=.003, aes(ymin=.data$lci, ymax=.data$uci), linewidth=1, position=position_dodge(width=posDodgeWidth)) +
        ...
        if(bothModels) {
            dcaPlotOverlay <-
                dcaPlot +
                geom_point(aes(y=.data$mn), size=3, position = position_dodge(width=posDodgeWidth)) +
                geom_errorbar(width=.003, aes(ymin=.data$min, ymax=.data$max), linewidth=1, position=position_dodge(width=posDodgeWidth)) +
                guides(color = guide_legend(override.aes = list(shape=NA)))
        } else {
            dcaPlotOverlay <-
                dcaPlot +
        ...
        if(bothModels) {
            dcaPlotOverlay <-
                dcaPlot +
                geom_point(aes(y=.data$mn), size=3, position = position_dodge(width=posDodgeWidth)) +
                geom_errorbar(width=.003, aes(ymin=.data$q1, ymax=.data$q3), linewidth=1, position=position_dodge(width=posDodgeWidth)) +
                guides(color = guide_legend(override.aes = list(shape=NA)))
        } else {
            dcaPlotOverlay <-
                dcaPlot +
